...
function getHashicorpPackages(directory, isRecursive = false) {
    let results = [];
    const dirToSearch = path_1.default.join(directory, 'node_modules', '@hashicorp');
    ...
    fs_1.default.readdirSync(dirToSearch).forEach((dir) => {
        if ((dir !== 'react-global-styles' && dir.startsWith('react-')) ||
            dir === 'nextjs-scripts')
            results.push(path_1.default.join(dirToSearch, dir));
        // call the function again to get nested deps
        results = results.concat(getHashicorpPackages(path_1.default.join(dirToSearch, dir), true));
    });
    if (isRecursive)
        return results;
    // if we're not a recursive call, resolve all of the paths relative to the first directory searched
    return results.map((dir) => path_1.default.isAbsolute(dir)
        ? path_1.default.relative(path_1.default.join(directory, 'node_modules'), dir)
        : dir);
}
