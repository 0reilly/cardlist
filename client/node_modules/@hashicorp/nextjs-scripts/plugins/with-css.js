...
module.exports = function withCss({ afterPlugins = [], beforePlugins = [], presetEnvOptions = {}, } = {}) {
    return function withCssInternal(nextConfig = {}) {
        ...
        return Object.assign({}, nextConfig, {
            webpack(config, options) {
                ...
                modifyLoaderConfig(config.module.rules, [/(?<!\.module)\.css$/, /\.module\.css$/], (rule) => {
                                ...
                                postcssPresetEnv(Object.assign({
                                    ...
                                    features: {
                                        ...
                                        'custom-media-queries': {
                                            importFrom: path.join(dir, 'node_modules/@hashicorp/mktg-global-styles/custom-media.css'),
                                        },
                ...
                if (!hasAppliedModifications)
                    throw new Error('nextjs-scripts - PostCSS loader modifications were not applied! This may have been caused by an issue in a newer version of next. Please reach out in #team-web-platform.');
...
function modifyLoaderConfig(rules, regexes, cb) {
    rules.map((rule) => {
        if (rule.oneOf)
            return modifyLoaderConfig(rule.oneOf, regexes, cb);
        regexes.map((regex) => {
            if (rule.test && rule.test.toString() === regex.toString())
                cb(rule);
        });
    });
}
